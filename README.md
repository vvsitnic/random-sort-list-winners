...
To safely bypass it, the netlify serverless service comes into clutch by providing proxy between the client and website.

## Instalation and deployment

Due to the fact that this app takes advantage of netlify serverless, simply running `npm run build` on a local machine will not work. For this reason the best option would be to connect netlify to your github repo or manually upload application files to netlify.
